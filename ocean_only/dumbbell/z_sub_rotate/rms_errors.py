...
pgeom=nc.Dataset('../z_rotate/ocean_geometry.nc')
...
Prog=nc.Dataset('../z_rotate/prog.nc')
...
def rv_rms():
    ...
    rms_err=diff.std()
    print('Vorticity error (10-4 s-1) = ',str(rms_err)[:5])
    return rms_err
...
def enstro_rms():
    ...
    rms_err=diff.std()
    print('Enstrophy error (10-7 s-2) = ',str(rms_err)[:5])
    return rms_err
...
def salt_rms():
    ...
    rms_err=rms.std()
    print('RMS Salinity error (psu) =', str(rms_err)[:5])
    return rms_err
...
g.write(str(rv_rms)[:5] +','+str(ens_rms)[:5]+'\n')
g.close()
